...
describe('Testing undefined values', () => {
    ...
    it('works with multiple', () => {
        ...
        expect(KeysKey.Is(event, KeysKey.Letter.A, KeysKey.ModifierKeys.Shift)).to.be.deep.equal(['A', 'Shift']);
    });

    it('returns nothing if one does not match', () => {
        const event = { key: 'A', metaKey: false, shiftKey: false, ctrlKey: false };
        expect(KeysKey.Is(event, KeysKey.Letter.A, KeysKey.ModifierKeys.Shift)).to.be.undefined;
    });
